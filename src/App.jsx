...
export default function App() {
     ...
     return (
        <div className="min-h-screen bg-slate-50 font-sans pb-20">
           <header className="bg-white p-4 shadow-sm flex justify-between items-center sticky top-0 z-10">
              <div className="flex items-center gap-2"><Monitor className="text-green-600"/><span className="font-black text-slate-800 text-lg">Gerencia</span></div>
           ...
           <div className="px-4 animate-in slide-in-from-bottom-4">
              ...
              {adminTab === 'menu' && <div className="space-y-4"><div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100"><h3 className="font-bold text-lg mb-4">Nuevo Plato</h3><div className="flex gap-3"><input placeholder="Nombre" value={newItemForm.name} onChange={e=>setNewItemForm({...newItemForm,name:e.target.value})} className="flex-[2] bg-slate-50 p-3 rounded-xl border outline-none"/><input placeholder="Precio" type="number" value={newItemForm.price} onChange={e=>setNewItemForm({...newItemForm,price:e.target.value})} className="flex-1 bg-slate-50 p-3 rounded-xl border outline-none"/><select value={newItemForm.category} onChange={e=>setNewItemForm({...newItemForm,category:e.target.value})} className="flex-1 bg-slate-50 p-3 rounded-xl border outline-none">{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select><button onClick={addMenuItem} className="bg-slate-900 text-white px-6 rounded-xl font-bold">Crear</button></div></div><div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">{menuItems.map(item => (<div key={item.id} className="p-4 border-b flex justify-between items-center hover:bg-slate-50"><div><span className="font-bold block">{item.name}</span><span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded mr-2">{item.category}</span><span className="text-sm font-bold text-blue-600">${item.price}</span></div><button onClick={()=>deleteMenuItem(item.id)} className="text-slate-300 hover:text-red-500"><Trash2/></button></div>))}</div></div>}
              {adminTab === 'finance' && <div className="space-y-4"><div className="grid grid-cols-1 gap-2"><KpiCard t="Ventas Históricas" v={salesTotal} c="green"/><KpiCard t="Gastos" v={expenses.reduce((a,b)=>a+b.amount,0)} c="red"/><KpiCard t="Ganancia Neta" v={netProfit} c="blue"/></div><div className="bg-white p-5 rounded-2xl shadow-sm"><h3 className="font-bold mb-3">Ranking Meseros</h3>{Object.entries(waiterStats).sort((a,b)=>b[1]-a[1]).map(([n,v],i)=>(<div key={n} className="flex justify-between border-b py-2 last:border-0"><span className="flex items-center gap-2"><span className="font-bold text-slate-400 text-xs w-5">#{i+1}</span> {n}</span><span className="font-bold text-slate-700">${v.toLocaleString()}</span></div>))}</div><div className="bg-white p-5 rounded-2xl shadow-sm"><h3 className="font-bold mb-3">Gastos</h3><div className="flex gap-2 mb-4"><input placeholder="Motivo" value={newExpense.desc} onChange={e=>setNewExpense({...newExpense,desc:e.target.value})} className="flex-[2] border p-2 rounded-lg"/><input type="number" placeholder="$" value={newExpense.amount} onChange={e=>setNewExpense({...newExpense,amount:e.target.value})} className="flex-1 border p-2 rounded-lg"/><button onClick={addExpense} className="bg-slate-800 text-white px-4 rounded-lg font-bold">+</button></div><div className="max-h-40 overflow-y-auto space-y-2">{expenses.map(e=>(<div key={e.id} className="flex justify-between text-sm border-b pb-1"><span>{e.description}</span><span className="text-red-500 font-bold">-${e.amount}</span></div>))}</div></div></div>}
              {adminTab === 'settings' && <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4"><h3 className="font-bold text-lg mb-2">Datos de Facturación</h3><div><label className="text-xs text-slate-400 uppercase font-bold">NIT</label><input className="w-full bg-slate-50 p-2 rounded border" value={settingsForm.nit} onChange={e=>setSettingsForm({...settingsForm, nit:e.target.value})}/></div><div><label className="text-xs text-slate-400 uppercase font-bold">Dirección</label><input className="w-full bg-slate-50 p-2 rounded border" value={settingsForm.address} onChange={e=>setSettingsForm({...settingsForm, address:e.target.value})}/></div><div><label className="text-xs text-slate-400 uppercase font-bold">Teléfono</label><input className="w-full bg-slate-50 p-2 rounded border" value={settingsForm.phone} onChange={e=>setSettingsForm({...settingsForm, phone:e.target.value})}/></div><div><label className="text-xs text-slate-400 uppercase font-bold">Pie de Página</label><input className="w-full bg-slate-50 p-2 rounded border" placeholder="Gracias por su compra" value={settingsForm.footer} onChange={e=>setSettingsForm({...settingsForm, footer:e.target.value})}/></div><div className="flex gap-2"><label className="text-xs text-slate-400 uppercase font-bold">Logo URL</label><input className="w-full bg-slate-50 p-2 rounded border" placeholder="https://..." value={settingsForm.logoUrl} onChange={e=>setSettingsForm({...settingsForm, logoUrl:e.target.value})}/></div><button onClick={saveSettings} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold">Guardar</button></div>}
           </div>
  ...
  return (
      <div className="p-4 bg-white min-h-screen pb-24 font-sans relative">
          ...
          <header className="flex justify-between items-center mb-4 bg-white sticky top-0 z-10 py-2"><div><h1 className="font-black flex items-center gap-2 text-lg text-slate-800"><User className="text-blue-600"/> {waiterName}</h1><span className="text-[10px] text-slate-400 font-bold uppercase">{company.id}</span></div><button onClick={exitRole} className="text-xs text-red-400 font-bold bg-red-50 p-2 rounded">SALIR</button></header>
